...
# ── Recording Stream + Download Views (R1+R2) ──────────────────────────

class CallRecordingStreamView(APIView):
    """
    GET /api/calls/<uuid>/recording/             → inline stream (HTML5 audio)
    GET /api/calls/<uuid>/recording/download/    → attachment download
    Supports HTTP Range requests for seeking.
    """
    permission_classes = [permissions.IsAuthenticated]

    def get(self, request, call_id, download=False):
        ...
        # ── Permission check ──
        u = request.user
        is_admin_sup = u.role in ('admin', 'supervisor')
        is_owner_agent = call.agent_id == u.id
        is_lead_agent = call.lead and call.lead.assigned_to_id == u.id
        if not (is_admin_sup or is_owner_agent or is_lead_agent):
            return Response({'detail': 'Not allowed'}, status=403)

        # ── Get the recording ──
        rec = call.recordings.order_by('-created_at').first()
        if not rec:
            # Try to find on disk by uniqueid
            from apps.calls.tasks import _scan_recording_for_call
        ...
        filepath = os.path.join(base, rec.filename)
        if not os.path.exists(filepath):
            return Response({'detail': f'File missing on disk: {rec.filename}'}, status=410)

        file_size = os.path.getsize(filepath)
        content_type = mimetypes.guess_type(filepath)[0] or 'audio/wav'
        ...
        resp['Accept-Ranges']  = 'bytes'
        if download:
            resp['Content-Disposition'] = f'attachment; filename="{os.path.basename(rec.filename)}"'
        else:
            resp['Content-Disposition'] = f'inline; filename="{os.path.basename(rec.filename)}"'
        return resp


class CallRecordingDownloadView(CallRecordingStreamView):
    def get(self, request, call_id):
        return super().get(request, call_id, download=True)
