...
class Disposition(TimeStampedModel):
    """Call outcome/result options configured by admin."""
...
class Call(BaseModel):
    ...
    wait_time      = models.PositiveIntegerField(default=0, help_text='Ring/wait seconds')
    ...
    recording_file = models.CharField(max_length=500, blank=True,
                                      help_text='Relative path on Asterisk server')
    recording_url  = models.URLField(blank=True,
                                     help_text='Full URL to download/stream recording')
...
class CallEvent(TimeStampedModel):
    """AMI-generated events attached to a call."""
    EVENT_CHOICES = [
        ('dial',       'Dial'),
        ('answer',     'Answer'),
        ('hangup',     'Hangup'),
