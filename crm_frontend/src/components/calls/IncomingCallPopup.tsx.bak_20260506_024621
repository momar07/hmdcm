...
export function IncomingCallPopup() {
  ...
  const handleDismiss = useCallback(() => {
    ...
    // 1. Log dismiss to backend FIRST (before SIP hangup → AMI timeout race)
    if (call?.call_id) {
      import('@/lib/api/calls').then(({ callsApi }) => {
        callsApi.dismissCall(call.call_id).catch((e: any) => {
          console.error('[Popup] dismissCall failed:', e?.response?.data || e);
        });
      });
    }
    // 2. Then hangup SIP
  ...
  const handleReject = useCallback(() => {
    const call = incomingCallRef.current;
    // 1. Log reject to backend FIRST
    if (call?.call_id) {
      import('@/lib/api/calls').then(({ callsApi }) => {
        callsApi.rejectCall(call.call_id).catch((e: any) => {
          console.error('[Popup] rejectCall failed:', e?.response?.data || e);
        });
      });
    }
    // 2. Then hangup SIP
    if (!actions) {
      console.warn('[Popup] Cannot reject — SIP actions not registered');
    } else {
      actions.hangup?.();
    }
    setVisible(false);
  ...
  // Report popup shown to backend for activity tracking
  useEffect(() => {
    const call = incomingCall;
    if (call?.call_id && visible && agentStatus !== 'away') {
      import('@/lib/api/calls').then(({ callsApi }) => {
        (callsApi as any).popupShown?.(call.call_id).catch(() => {});
      });
    }
  }, [incomingCall?.call_id, visible, agentStatus]);
  ...
  // HOISTED: derived call data — must exist before the effects below
  ...
  // (wsData / spData / hasWs / isKnownLead are hoisted above)
  let leadName:    string;
