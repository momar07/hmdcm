    ...
    <Modal open onClose={onClose} title="WhatsApp Message" size="sm">
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm
                       resize-none focus:outline-none focus:ring-2 focus:ring-green-300"
          />
        </div>
        ...
        <div className="flex gap-2 pt-2 border-t border-gray-100">
          ...
          {note.trim() ? (
            <Button
              variant="primary" loading={isPending}
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-2 justify-end pt-2 border-t border-gray-100">
          ...
          <Button variant="primary" icon={<RefreshCw size={14} />}
                  loading={isPending} disabled={!date || !time}
                  onClick={() => mutate()}>
            Reschedule
          </Button>
        </div>
  ...
  return (
    <div className={`bg-white rounded-xl border shadow-sm p-4 flex flex-col gap-3
      ...
      {(f.customer_name || f.customer_phone) && (
        <div className="flex items-center gap-2 p-2.5 bg-gray-50 rounded-lg border border-gray-100">
          ...
          <div className="min-w-0 flex-1">
            {f.customer_name && (
              <p className="text-xs font-semibold text-gray-800 truncate">{f.customer_name}</p>
            )}
            {f.customer_phone && (
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
        {f.lead_title && (
          <span className="flex items-center gap-1"><Users size={11} />{f.lead_title}</span>
        )}
        ...
        {f.assigned_to_name && (
          <span className="flex items-center gap-1">👤 {f.assigned_to_name}</span>
        )}
      </div>
        ...
        <div className="flex flex-col gap-2 pt-1 border-t border-gray-100">
          ...
          <div className="flex gap-2">
            <Button variant="success" size="sm" icon={<CheckCircle size={13} />}
                    ...
                    onClick={() => onComplete(f.id)} className="flex-1">
              Done
            </Button>
...
export default function FollowupsPage() {
  ...
  // accurate overdue + upcoming counts from dedicated endpoints
  ...
  const completeMutation = useMutation({
    ...
    onSuccess:  () => { toast.success('Completed ✅'); qc.invalidateQueries({ queryKey: ['followups'] }); },
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess:  () => { toast.success('Cancelled'); qc.invalidateQueries({ queryKey: ['followups'] }); },
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-white rounded-xl border border-gray-200
                                    shadow-sm p-4 animate-pulse h-48" />
          ))}
        </div>
      ) : results.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-1">
            {statusFilter === 'pending' ? 'Great! No pending follow-ups.' : 'Try changing the filters.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {whatsapping && (
        <WhatsAppModal followup={whatsapping} onClose={() => setWhatsapping(null)} />
      )}
    </div>
  );
}
