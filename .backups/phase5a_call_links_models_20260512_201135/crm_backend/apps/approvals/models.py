...
class ApprovalCallLink(models.Model):
    """Many-to-many link between ApprovalRequest and Call with audit metadata."""
    ...
    approval = models.ForeignKey(
        ApprovalRequest, on_delete=models.CASCADE, related_name="call_links",
    )
    call = models.ForeignKey(
        "calls.Call", on_delete=models.CASCADE, related_name="approval_links",
    )
    ...
    linked_by = models.ForeignKey(
        ...
        related_name="approval_call_links_created",
    )
    unlinked_at = models.DateTimeField(null=True, blank=True)
    unlinked_by = models.ForeignKey(
        "users.User", on_delete=models.SET_NULL, null=True, blank=True,
        related_name="approval_call_links_unlinked",
    )
    ...
    class Meta:
        ...
        ordering = ["-linked_at"]
        indexes = [
            models.Index(fields=["approval", "unlinked_at"]),
            models.Index(fields=["call"]),
        ]

    def __str__(self):
        return f"ApprovalCallLink({self.approval_id} ↔ {self.call_id})"
