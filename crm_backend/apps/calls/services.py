...
def find_lead_by_phone(phone_number: str):
    ...
    normalized = _re.sub(r'[^0-9+]', '', phone_number)

    # 1. Exact match on phone field
    lead = Lead.objects.filter(phone=phone_number, is_active=True).first()
    if lead:
        return lead

    # 2. Normalized match
    if normalized != phone_number:
        lead = Lead.objects.filter(phone=normalized, is_active=True).first()
        if lead:
            return lead

    # 3. Suffix match (last 9 digits) for local vs international
    if len(normalized) >= 9:
        suffix = normalized[-9:]
...
def get_or_create_lead_for_call(phone_number: str, direction: str = 'inbound',
                                 agent=None, caller_name: str = ''):
    """
    Called when a new call arrives (Screen Pop).
    1. Search for existing Lead by phone.
    2. If found  → return (lead, False)
    3. If not    → auto-create a new Lead and return (lead, True)
    Never creates a Customer directly — Lead is the primary entity.
    """
    ...
    # Auto-create new lead from call
    # Parse name if provided (e.g. "John Doe")
    ...
    lead = create_lead(
        data  = {
            'title'      : f'Lead from {direction} call - {phone_number}',
            'phone'      : phone_number,
...
def link_call_to_lead(call_id, lead_id, actor=None):
    """
    Link an existing Call record to a Lead.
    """
    ...
    LeadEvent.objects.create(
        ...
        event_type = 'note',
        actor      = actor,
        note       = f'Call linked: {call.caller} → {call.callee}',
    )
    return call
